...
#define FAN_50P_TEMP            87  //temp at which fan will be spinning at it's 50% speed.
...
//            cout << "1! FAN IS OFF";
            ...
            // slider = 1 when tmp = FAN_OFF_TEMP => fanSpeed = 25%
            float slider = ( float(temp - FAN_OFF_TEMP) / float(FAN_25P_TEMP - FAN_OFF_TEMP) );
            fanSpeed = unperc(25 * slider);
//            cout << "2! FAN IS "<<fanSpeed;
        }
        else if (temp <= FAN_50P_TEMP){
            // slider = 1 when tmp = FAN_50P_TEMP => fanSpeed = 50%
